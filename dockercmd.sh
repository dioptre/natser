...
# NATSER
# aws ssm get-parameters --names $SSM_ID_NATSER_CERT --no-with-decryption --region $AWS_REGION --output json | jq -r '.Parameters[0] | .Value' > /root/natser/server.crt
# aws ssm get-parameters --names $SSM_ID_NATSER_KEY --with-decryption --region $AWS_REGION --output json | jq -r '.Parameters[0] | .Value' > /root/natser/server.key
